...
- Committed-source capture: `npm run cli -- acceptance capture-source <profile-id> <repo> <commit> [--json] [--bundle-dir <dir>]` (VS spec layer 3d; Git object reads of a pinned commit; engine-owned `package-manifest-lockfile` profile; not live execution; not verified single-milestone execution completed)
