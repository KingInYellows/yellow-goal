...
- Releases (`../.github/workflows/release.yml`): an annotated `v*` tag (`git tag -a v0.1.0 -m v0.1.0`) that matches `package.json` version and peels to `HEAD` re-runs those gates and attaches `goal-gen-<ver>.tgz` as a GitHub Release asset (unmetered; never `actions/upload-artifact`). Packing runs with `GH_TOKEN` absent; the token is scoped only to idempotent Release publication, which verifies asset SHA-256 before publishing a draft. To recover an existing tag after a partial publish, dispatch Release from `main` with `-f tag=v0.1.0 -f commit=<expected-40-character-commit>`; the workflow checks out that immutable tag, verifies the expected peeled commit, and never retags it. Consumers pin that URL. Do not cut a tag from an autonomous session.
