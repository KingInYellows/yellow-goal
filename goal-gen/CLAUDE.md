...
- Releases (`../.github/workflows/release.yml`): an annotated `v*` tag (`git tag -a v0.1.0 -m v0.1.0`) that matches `package.json` version and peels to `HEAD` re-runs those gates and attaches `goal-gen-<ver>.tgz` as a GitHub Release asset (unmetered; never `actions/upload-artifact`). Packing runs with `GH_TOKEN` absent; the token is scoped only to idempotent Release publication, which verifies asset SHA-256 before publishing a draft. Consumers pin that URL. Do not cut a tag from an autonomous session.
