...
- [ ] 4.5: Termination: after each verify pass, `satisfies(currentState, goalSpec.goalState)` → `succeeded`. Emit a final structured run summary (status, cost, replans, re-extractions, per-action outcomes). **Sign-off gap (real code gap in PR #8):** when `completionPolicy` is `verify+signoff` or `operator-defined`, specs and ADR-0008 require an awaiting-acceptance prompt after `goalState` is satisfied; the shipped orchestrator normalizes `operator-defined` → `verify+signoff` at the DoD gate but does not implement a post-execution sign-off prompt — it marks `succeeded` unconditionally. A second `stdinConfirm`-style prompt after termination is needed for `verify+signoff` goals; this is deferred but must be addressed before enabling operator-judged goals in production. **[RESOLVED 2026-08-26 — closed by PR #12 (`2d827a3`, gate & control mechanics), verified against code:** on `verify+signoff`/`operator-defined` policies the orchestrator now awaits `persistAwaitingAcceptance()` (status → `'awaiting-acceptance'` + durable `AwaitingAcceptance` event write, both awaited per R31) and then awaits `acceptanceGate` before terminating (`backend/src/orchestrator/orchestrator.ts:319-328`, `:917-924`), with a default `stdinAcceptanceGate` (`:203`) preserving CLI behavior and a rejection → remediation path (`signoffRemediationActive`). See `plans/m1-backend-api-persistence-controls-02-gate-control-mechanics.md` (R29–R31).]
...
- [ ] 5. agent/test/m1-runner-e2e (probe committed 2026-06-29; **stays open** until the operator-gated one-time real-`claude` end-to-end run passes the acceptance gate — the probe is excluded from CI, so committing it does not by itself satisfy the gate). *Deferred 2026-08-26: to be run in a future interactive session with the operator present (real spend, human approval required). Tracked in the harness coordination workspace ledger (`workspace-meta/next-task.md`, outside this repo). Does not block the CI-gates or request-to-run pipeline work.*
