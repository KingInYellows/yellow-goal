...
| Capture or overlay persist of dest `stash/mid/a/b/leaf` with only `stash` existing after `stash` is moved into the captured source when `mid` appears | n/a | not launched | no success (`USAGE_ERROR`); created dest components are mkdir'd under a unique temporary name and opened before rename onto the final child name so `createdFds` holds the inode this process created before the final name is visible; publish uses no-replace `renameat2` (`RENAME_NOREPLACE`) loaded from the process's mapped libc in `/proc/self/maps` (not hard-coded `libc.so.6` / `libc.so` sonames) so a concurrent empty directory at the final name is not unlinked — EEXIST rmdirs the held tmp and opens the existing child as not-created; a mkdir/open swap of the final name cannot make rollback delete a replacement and leave the original inode; created dest components are rolled back in reverse order so leftover `mid`/`a`/`b`/`leaf` dirs are absent from source; pre-existing `stash` is not removed; COMPLETE/manifest/blobs absent; dest-inside-source stays `USAGE_ERROR`; honest missing-parent dest outside source still persist `COMPLETE`; rollback removes the inode held by each created fd through its current parent (inode-verified), not a `realpath` pathname that a replacement empty directory can occupy | 2 / 0 |
